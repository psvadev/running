...
- **Løpeatlas** — a motivational travel section (grew out of "Land løpt i"): a summary line (land · verdensdeler N/6 · utenlandsøkter · distanse i utlandet · lengste utenlands), a flag-chip strip of every country you've run in (flag + Norwegian name + all-time run count, Norway first, click a chip for that country's sessions), a continent-collection line (Europa ✓ / Afrika 🔒 …), and ~13 auto-derived **travel achievements** grouped Start / Land / Verdensdeler / Norden / Erfaring with progress bars on locked ones (First Steps Abroad, Passport Stamp, World Traveler, Continental Explorer, Nordic Complete, International 5K/10K, Scenic Route, …). Everything is derived from your runs + events — nothing to manage; country-collection badges count foreign countries only. Empty-`land` runs count as Norway (so only abroad runs need the Land field set)

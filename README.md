...
- **Land** *(optional)* — the country a run took place in; leave it blank for home runs (empty means Norway) and only fill it for runs abroad. Norwegian country-name autocomplete; the field is prefilled from Strava when a fetched activity carries a country. Feeds the "Løpeatlas" card on the dashboard
...
- **Aerob effektivitet** — aerobic efficiency trend for Easy sessions (speed ÷ heart rate), with rolling average and personal average reference line; venue toggle (Alle / Utendørs / Tredemølle) to slice by run type; **Sone 2** filter to isolate only runs where avg HR (or ≥70% of zone time) falls within configured Zone 2 — gives a cleaner aerobic signal by excluding Easy runs that drifted too hard
...
- **Løpeatlas** — a motivational travel section (grew out of "Land løpt i"): a summary line (land · verdensdeler N/6 · utenlandsøkter · distanse i utlandet · lengste utenlands), a flag-chip strip of every country you've run in (flag + Norwegian name + all-time run count, Norway first, click a chip for that country's sessions), a continent-collection line (Europa ✓ / Afrika 🔒 …), and **30+ auto-derived travel achievements** grouped Start / Land / Verdensdeler / Norden / Distanse / Stil / Sesong with progress bars on locked ones — foreign-country tiers (Passport Stamp → World Traveler → Frequent Flyer → Globetrotter), per-continent regional explorers (European Tour, Asia/Americas/Africa Explorer) + Continental Explorer / Global Runner / Cross-Continental / Both Hemispheres, Nordic Runner/Complete, distance milestones abroad (5K/10K/Half/Marathon, Globe Miler, Foreign Racer), style (Scenic Route, Hotel Treadmill, Outdoor Explorer, Foreign Hills) and season/time (Winter Escape, New Year Abroad, Multi-Year Traveler, …). Everything is derived from your runs + events — nothing to manage; country-collection badges count foreign countries only, and tiered ladders show just your next goal (no redundant 3/5, 3/10 stacking). Click a flag chip to open that country's runs and jot a free-text **memory/note** for the place (📝) — the only thing you type; everything else is derived. Empty-`land` runs count as Norway (so only abroad runs need the Land field set)

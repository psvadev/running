...
- **Land** *(optional)* — the country a run took place in; leave it blank for home runs (empty means Norway) and only fill it for runs abroad. Norwegian country-name autocomplete; the field is prefilled from Strava when a fetched activity carries a country. Feeds the "Land løpt i" card on the dashboard
...
- **Land løpt i** — a motivational flag-chip strip of every country you've run in: flag (from flagcdn.com) + Norwegian country name + all-time run count, Norway first; click a chip to see that country's sessions; empty-`land` runs count as Norway (so only abroad runs need the Land field set)

...
- **Land** *(optional)* — the country a run took place in; leave it blank for home runs (empty means Norway) and only fill it for runs abroad. Norwegian country-name autocomplete; the field is prefilled from Strava when a fetched activity carries a country. Feeds the "Land løpt i" strip in Rekorder

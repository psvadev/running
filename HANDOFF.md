...
| `renderEfficiencyChart(sessions)` | Aerobic efficiency chart — filters to Easy sessions with HR data; `snittkmh` derived on-the-fly from `distanse/varighet` if not stored (fixes imported sessions missing the field); venue filter read from global `aeVenueFilter` ('all'/'outdoor'/'indoor'), controlled by **Alle/Utendørs/Tredemølle** toggle pills; computes score = `snittkmh/gjsnittspuls×100`; renders points + 4-session rolling avg + personal average reference line; empty-state message reflects active venue filter when fewer than 2 sessions qualify. |
| `renderZ2PaceChart(sessions)` | Zone-2 pace time-series — filters Easy + Langtur sessions (or single type via `z2PaceTypeFilter` toggle: Alle/Easy/Langtur); renders pace in seconds on an **inverted Y-axis** with `secsToMmSs` tick labels so improving pace trends upward; 2 datasets: raw pace points + 4-session rolling average; tooltip shows date, name, type, pace, HR, snittkmh; empty-state when fewer than 2 qualifying sessions. |
...
| `renderPaceHrChart(sessions)` | Pace vs HR scatter — X=`gjsnittspuls`, Y=`tempo` (seconds/km); **Y-axis inverted** with `secsToMmSs` ticks so faster pace appears higher; same per-type color coding, regression line, legend, and toggle pattern as `renderSleepHrChart` (`paceHrTypeFilter`); tooltip shows type, tempo, puls, snittkmh. |

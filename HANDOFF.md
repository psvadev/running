...
- Charts (all **week-based**, not per-session — except Records). **Card order** (top to bottom): Årsmål → Rekorder → **Innsikter** → **Treningsrytme** → PMC → Belastning → Ukentlig distanse + Tempo → **Plan vs faktisk** → Treningskalender → **Ute vs inne** → Årssammenligning → Aerob effektivitet → Pulssoner → **Sko oversikt** + Ukentlig oversikt
  ...
  - **Tempo per uke** — line chart, weighted average pace per week (total time ÷ total km). Respects global `paceUnit` toggle (min/km ↔ km/t); Y-axis inverted in pace mode, normal in km/h mode. Supports event marker lines. **Tooltip shows:** pace/speed + km/løp/tid. **Click a point → `DetailPanel.openWeek`**. Uses `_rawLabels` (YYYY-WW) for event matching.

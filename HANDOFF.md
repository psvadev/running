...
- Charts (all **week-based**, not per-session — except Records). **Card order** (top to bottom): Årsmål → Rekorder → **Innsikter** → **Treningsrytme** → PMC → Belastning → Ukentlig distanse + Tempo → **Plan vs faktisk** → Treningskalender → **Ute vs inne** → Årssammenligning → Aerob effektivitet → Pulssoner → **Sko oversikt** + Ukentlig oversikt
  - **Rekorder** — volume row (total dist, total time, avg km/week, longest session dist+time, best week, best month, best 4-week block, **Mest høydemeter**) + quality row (best pace, best km/h, **Beste Easy/Long/Tempo/Test pace** — all four use "pace" label; best aerob eff., lowest avg HR, longest streak, current streak) + **Distanse-PR** sub-section (5 km, 10 km, Halvmaraton, Maraton — fastest session per bracket). `computeRecords()` returns `bestEasyPace`, `bestLangturPace`, `bestTempoPace`, `bestTestPace`, `bestHoyde`.

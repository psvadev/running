...
- **Løpeatlas** (travel stats + 36 achievements in 6 groups: Milepæler/Land/Verdensdeler/Distanse/Omgivelser/Sesong) is **fully derived** from `sessions` (`land`/`dato`/`distanse`/`løpetype`/`okttype`/`hoydeMeter`) + `events` (`vacation` overlap for the Vacation Runner badge) via `computeAtlas()` — **nothing persisted**, no badge state, so editing an old run's date/country can shift derived unlock dates ("Sist låst opp"). Home = `ATLAS_HOME='NO'`; blank `land` = home. Country-collection tier badges (Passport/World Traveler/Frequent Flyer/Globetrotter) count foreign countries only; regional-explorer + continents + Nordic count all resolved incl. home via the static `CONTINENTS` map (6 inhabited; Antarctica excluded; Both Hemispheres uses a `SOUTHERN` hand-list); unresolvable `land` text still shows as a chip but has no continent. The two **Nordic** badges live under Verdensdeler (a near-home European sub-region — no standalone Norden group). Tiered **ladders** (`ATLAS_LADDERS`: foreign-country count, continents, Nordic, longest-single-run distance, cumulative km abroad `globe-*` 10→25→50→100→250, and distinct outdoor-abroad countries `outdoor-*` 3→5→10) show unlocked tiers + only the next locked one, so a beginner starts on an obtainable target that escalates once reached. A few badges carry **dynamic** names/desc from the derived data: New Country Year shows the year + which country (`New Country Year (2026)` / `Første nye land i 2026: Sverige`), Multi-Year Traveler lists the actual years (`… i 2 kalenderår: 2025, 2026`). Single-run/threshold badges also carry the **country where they were earned** (`country` ISO code in the badge object) and render a small flag + country name on the "Låst opp" line; continent-count (Continental/Global), cumulative-km (`globe-*`), Multi-Year, and New Country Year (already named in its desc) deliberately carry no flag.

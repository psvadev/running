...
- `shoes` — `{ name, retirementKm?, retired?, obtained? }`. `obtained` = purchase month `YYYY-MM` (month picker); drives the shoe's displayed **age** on its Innstillinger card. Editable on existing shoes via the card's Rediger panel.
